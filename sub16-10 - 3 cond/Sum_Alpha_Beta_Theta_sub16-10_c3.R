...
# 1st condition
sum_alpha_theta_beta_16sub_1=readMat('sum_alpha_theta_beta_16sub_cond_1.mat')$sum.alpha.theta.beta.sub
sum_alpha_theta_beta_16sub_1=as.vector(sum_alpha_theta_beta_16sub_1)
# 2nd condition
sum_alpha_theta_beta_16sub_2=readMat('sum_alpha_theta_beta_16sub_cond_2.mat')$sum.alpha.theta.beta.sub
sum_alpha_theta_beta_16sub_2=as.vector(sum_alpha_theta_beta_16sub_2)
# 3rd condition
sum_alpha_theta_beta_16sub_3=readMat('sum_alpha_theta_beta_16sub_cond_3.mat')$sum.alpha.theta.beta.sub
sum_alpha_theta_beta_16sub_3=as.vector(sum_alpha_theta_beta_16sub_3)

## 2 group - Control/Sea (10 subjects)
# 1st condition
sum_alpha_theta_beta_10Csub_1=readMat('sum_alpha_theta_beta_10Csub_cond_1.mat')$sum.alpha.theta.beta.sub 
sum_alpha_theta_beta_10Csub_1=as.vector(sum_alpha_theta_beta_10Csub_1)
# 2nd condition
sum_alpha_theta_beta_10Csub_2=readMat('sum_alpha_theta_beta_10Csub_cond_2.mat')$sum.alpha.theta.beta.sub
sum_alpha_theta_beta_10Csub_2=as.vector(sum_alpha_theta_beta_10Csub_2)
# 3rd condition
sum_alpha_theta_beta_10Csub_3=readMat('sum_alpha_theta_beta_10Csub_cond_3.mat')$sum.alpha.theta.beta.sub
sum_alpha_theta_beta_10Csub_3=as.vector(sum_alpha_theta_beta_10Csub_3)
...
c1_Before <- c(sum_alpha_theta_beta_16sub_1, sum_alpha_theta_beta_10Csub_1)
c2_During <- c(sum_alpha_theta_beta_16sub_2, sum_alpha_theta_beta_10Csub_2)
c3_After <- c(sum_alpha_theta_beta_16sub_3, sum_alpha_theta_beta_10Csub_3)

df_sum_alpha_theta_beta_16_10 <- data.frame(id, group, c1_Before, c2_During, c3_After)

# Gather the columns c1_Before and c2_After into long format.
# Convert id and cond into factor variables
df_sum_alpha_theta_beta_16_10 <- df_sum_alpha_theta_beta_16_10 %>%
...
df_sum_alpha_theta_beta_16_10$score <- df_sum_alpha_theta_beta_16_10$score*1e13

# Inspect some random rows of the data by groups
set.seed(123)
df_sum_alpha_theta_beta_16_10 %>% sample_n_by(group, cond, size = 1)
...
#sum.table <- summary(df_sum_alpha_theta_beta_16_10)
...
# Create box plots:
bxp <- ggboxplot(
  df_sum_alpha_theta_beta_16_10, x = "cond", y = "score",
...
out=df_sum_alpha_theta_beta_16_10 %>%
  group_by(cond, group) %>%
  identify_outliers(score)
out
# Remove just one outlier (id = 8 & 18) from Groups 1 & 2
#df_sum_alpha_theta_beta_16_10 <- df_sum_alpha_theta_beta_16_10[-c(8,18,34,44), ]
# to Remove all the outliers
#tbr=which(df_sum_alpha_theta_beta_16_10$score%in%out$score)
#df_sum_alpha_theta_beta_16_10=df_sum_alpha_theta_beta_16_10[-tbr,]
...
ggqqplot(df_sum_alpha_theta_beta_16_10, "score", ggtheme = theme_bw()) +
...
res.aov <- anova_test(
  data = df_sum_alpha_theta_beta_16_10, dv = score, wid = id,
...
library(nparLD)
np_anova_model<-nparLD(score ~ cond * group, data = df_sum_alpha_theta_beta_16_10,
...
# on the Alpha+Beta+Theta score in each Group

# One-Way Anova - simple main effect of Cond variable
one.way2 <- df_sum_alpha_theta_beta_16_10 %>%
  group_by(group) %>%
...
ttest.p <- df_sum_alpha_theta_beta_16_10 %>%
  group_by(group) %>%
  pairwise_t_test(
...
ggbarplot(df_sum_alpha_theta_beta_16_10, x = "cond", y = "score", 
          ylab='mean Alpha+Beta+Theta score', fill='group',
          add = c("mean_se"),
...
# on the Alpha+Beta+Theta score in each ??ondition

# One-Way Anova - simple main effect of the Group variable
one.way <- df_sum_alpha_theta_beta_16_10 %>%
  group_by(cond) %>%
...
ttest <- df_sum_alpha_theta_beta_16_10 %>%
  group_by(cond) %>%
  pairwise_t_test(
...
# Bar-plot for 2 grouping variables
ggbarplot(df_sum_alpha_theta_beta_16_10, x = "group", y = "score", 
          ylab='mean Alpha+Beta+Theta score', fill='cond',
          add = c("mean_se"),
          error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
  #geom_jitter(size=1, width=0.1)+ # add subjects
...
one.way.np <- df_sum_alpha_theta_beta_16_10 %>%
  group_by(group) %>%
  kruskal_test(score ~ cond)
one.way.np

# Effect size of the Cond on Alpha+Beta+Theta score
df_sum_alpha_theta_beta_16_10 %>%
  group_by(group) %>%
  kruskal_effsize(score ~ cond)

# Paired Wilcoxon test - Cond 1 vs Cond 2 vs Cond 3 in Group 1 & 2
Wlx.test.p <- df_sum_alpha_theta_beta_16_10 %>%
  group_by(group) %>%
...
# Bar-plot for 2 grouping variables
ggbarplot(df_sum_alpha_theta_beta_16_10, x = "cond", y = "score", 
          ylab='median Alpha+Beta+Theta score', fill='group',
          add = c("median"),
          #error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
  scale_fill_manual(values=group.colors)+ #set colors
...
# We investigate the effect of the between-subject factor "Group"
# on the Alpha+Beta+Theta score in each condition

# Kruskal-Wallis test 2
one.way.np2 <- df_sum_alpha_theta_beta_16_10 %>%
  group_by(cond) %>%
  kruskal.test(score ~ group, paired = FALSE)
one.way.np2

# Effect size of the Group on Alpha+Beta+Theta score
df_sum_alpha_theta_beta_16_10 %>%
  #group_by(cond) %>%
  kruskal_effsize(score ~ group)

# Non-paired Wilcoxon test - Group 1 vs Group 2 in Cond 1, 2 & 3
Wlx.test <- df_sum_alpha_theta_beta_16_10 %>%
  group_by(cond) %>%
  wilcox_test(
    score ~ group, paired = FALSE,
    p.adjust.method = "BH"
...
# Bar-plot for 2 grouping variables
ggbarplot(df_sum_alpha_theta_beta_16_10, x = "group", y = "score", 
          ylab='median Alpha+Beta+Theta score', fill='cond',
          add = c("median"),
          #error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
  #geom_jitter(size=1, width=0.1)+ # add subjects
...
ggbarplot(df_sum_alpha_theta_beta_16_10, x = "cond", y = "score", 
          ylab='median Alpha+Beta+Theta score', 
          fill=cond.colors,
          add = c("median"),
          error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
...
c1_Before_Hypno <- c(sum_alpha_theta_beta_16sub_1)
c2_During_Hypno <- c(sum_alpha_theta_beta_16sub_2)
c3_After_Hypno <- c(sum_alpha_theta_beta_16sub_3)
c1_Before_Ctrl <- c(sum_alpha_theta_beta_10Csub_1)
c2_During_Ctrl <- c(sum_alpha_theta_beta_10Csub_2)
c3_After_Ctrl <- c(sum_alpha_theta_beta_10Csub_3)
...
# Data preparation
df_sum_alpha_theta_beta_16_10_g1 <- data.frame(idg1, c1_Before_Hypno, c2_During_Hypno, c3_After_Hypno) # create df
df_sum_alpha_theta_beta_16_10_g1 <- df_sum_alpha_theta_beta_16_10_g1 %>% # convert df
  gather(key = "cond", value = "score", c1_Before_Hypno, c2_During_Hypno, c3_After_Hypno) %>%
  convert_as_factor(idg1, cond)
#df_sum_alpha_theta_beta_16_10_g1 <- df_sum_alpha_theta_beta_16_10_g1[-c(18), ] # remove outlier-18
df_sum_alpha_theta_beta_16_10_g1$score <- df_sum_alpha_theta_beta_16_10_g1$score*1e13 # normalization

# Summary stats
df_sum_alpha_theta_beta_16_10_g1 %>%
  group_by(cond) %>%
  get_summary_stats(score, type = "mean_sd")
...
ggqqplot(df_sum_alpha_theta_beta_16_10_g1, x = "score", facet.by = "cond")
# Check the equality of variances
df_sum_alpha_theta_beta_16_10_g1 %>% levene_test(score ~ cond) # p should be >0.05
...
# One-Way Anova - simple main effect of Cond variable
one.way.g1 <- df_sum_alpha_theta_beta_16_10_g1 %>%
...
ttest.g1 <- df_sum_alpha_theta_beta_16_10_g1 %>% 
  t_test(score ~ cond, paired = TRUE) %>%
  add_significance()
ttest.g1

# Effect size
df_sum_alpha_theta_beta_16_10_g1 %>% cohens_d(score ~ cond, paired = TRUE, var.equal = FALSE)
...
ggbarplot(df_sum_alpha_theta_beta_16_10_g1, x = "cond", y = "score", 
          ylab='mean Alpha+Beta+Theta score', fill=cond.colors_hyp,
          add = c("mean_se"),
          error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
  #geom_jitter(size=1, width=0.1)+ # add subjects
...
ggpaired(df_sum_alpha_theta_beta_16_10_g1, x = "cond", y = "score", 
         fill=cond.colors_hyp,
         point.size=1.5,line.color = "gray", line.size = 0.4)+
  ylab('Alpha+Beta+Theta score')+
...
# Kruskal-Wallis test 1
one.way.np.g1 <- df_sum_alpha_theta_beta_16_10_g1 %>%
  kruskal_test(score ~ cond)
one.way.np.g1

# Effect size of the Cond on Alpha+Beta+Theta score in Group 1
df_sum_alpha_theta_beta_16_10_g1 %>%
  kruskal_effsize(score ~ cond)

# Wilcoxon test
Wlx.test.g1 <- df_sum_alpha_theta_beta_16_10_g1 %>% 
...
# Effect size
df_sum_alpha_theta_beta_16_10_g1 %>% wilcox_effsize(score ~ cond)

# Bar-plot for 1 grouping variable
ggbarplot(df_sum_alpha_theta_beta_16_10_g1, x = "cond", y = "score", 
          ylab='median Alpha+Beta+Theta score', fill=cond.colors_hyp,
          add = c("median"),
          error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
  #geom_jitter(size=1, width=0.1)+ # add subjects
...
# Data preparation
df_sum_alpha_theta_beta_16_10_g2 <- data.frame(idg2, c1_Before_Ctrl, c2_During_Ctrl, c3_After_Ctrl) # create df
df_sum_alpha_theta_beta_16_10_g2 <- df_sum_alpha_theta_beta_16_10_g2 %>% # convert df
  gather(key = "cond", value = "score", c1_Before_Ctrl, c2_During_Ctrl, c3_After_Ctrl) %>%
  convert_as_factor(idg2, cond)
#df_sum_alpha_theta_beta_16_10_g1 <- df_sum_alpha_theta_beta_16_10_g1[-c(18), ] # remove outlier-18
df_sum_alpha_theta_beta_16_10_g2$score <- df_sum_alpha_theta_beta_16_10_g2$score*1e13 # normalization

# Summary stats
df_sum_alpha_theta_beta_16_10_g2 %>%
  group_by(cond) %>%
  get_summary_stats(score, type = "mean_sd")
...
df_sum_alpha_theta_beta_16_10_g2 %>%
  ...
  shapiro_test(score)
# Draw a qq plot by cond
ggqqplot(df_sum_alpha_theta_beta_16_10_g2, x = "score", facet.by = "cond")
# Check the equality of variances
df_sum_alpha_theta_beta_16_10_g2 %>% levene_test(score ~ cond) # p should be >0.05
...
one.way.g2 <- df_sum_alpha_theta_beta_16_10_g2 %>%
...
# Compute Weltch T-test, which is the safer one
ttest.g2 <- df_sum_alpha_theta_beta_16_10_g2 %>% 
  t_test(score ~ cond, paired = TRUE) %>%
  add_significance()
ttest.g2

# Effect size
df_sum_alpha_theta_beta_16_10_g2 %>% cohens_d(score ~ cond, paired = TRUE, var.equal = FALSE)

# Report
# Auto-compute p-value label positions
ttest.g2 = ttest.g2 %>% add_xy_position(x = "cond")
# Bar-plot for 1 grouping variable
ggbarplot(df_sum_alpha_theta_beta_16_10_g2, x = "cond", y = "score", 
          ylab='mean Alpha+Beta+Theta score', fill=cond.colors_ctrl,
          add = c("mean_se"),
          error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
  geom_jitter(size=1, width=0.1)+ # add subjects
...
# Visualization for paired samples
ggpaired(df_sum_alpha_theta_beta_16_10_g2, x = "cond", y = "score", 
...
# Kruskal-Wallis test 2
one.way.np.g2 <- df_sum_alpha_theta_beta_16_10_g2 %>%
  kruskal_test(score ~ cond)
one.way.np.g2

# Effect size of the Cond on Alpha+Beta+Theta score in Group 2
df_sum_alpha_theta_beta_16_10_g2 %>%
  kruskal_effsize(score ~ cond)

# Wilcoxon test
Wlx.test.g2 <- df_sum_alpha_theta_beta_16_10_g2 %>% 
  wilcox_test(score ~ cond, paired = TRUE,
              p.adjust.method = "BH") %>%
  add_significance()
Wlx.test.g2
...
# Effect size
df_sum_alpha_theta_beta_16_10_g2 %>% wilcox_effsize(score ~ cond)

# Bar-plot for 1 grouping variable
ggbarplot(df_sum_alpha_theta_beta_16_10_g2, x = "cond", y = "score", 
          ylab='median Alpha+Beta+Theta score', fill=cond.colors_ctrl,
          add = c("median"),
          error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
  #geom_jitter(size=1, width=0.1)+ # add subjects
...
df_sum_alpha_theta_beta_16_10_c1 <- data.frame(id, group, c1_Before) # create df
names(df_sum_alpha_theta_beta_16_10_c1)[names(df_sum_alpha_theta_beta_16_10_c1) == 
                                    'c1_Before'] <- 'score'
#df_sum_alpha_theta_beta_16_10_c1 <- df_sum_alpha_theta_beta_16_10_c1[-c(18), ] # remove outlier-18
df_sum_alpha_theta_beta_16_10_c1$score <- df_sum_alpha_theta_beta_16_10_c1$score*1e13 # normalization

# Summary stats
df_sum_alpha_theta_beta_16_10_c1 %>%
  group_by(group) %>%
  get_summary_stats(score, type = "mean_sd")
...
ggqqplot(df_sum_alpha_theta_beta_16_10_c1, x = "score", facet.by = "group")
# Check the equality of variances
df_sum_alpha_theta_beta_16_10_c1 %>% levene_test(score ~ group) # p should be >0.05
...
# Compute Weltch T-test, which is the safer one
ttest.c1 <- df_sum_alpha_theta_beta_16_10_c1 %>% 
  t_test(score ~ group, paired = FALSE) %>%
  add_significance()
ttest.c1

# Effect size
df_sum_alpha_theta_beta_16_10_c1 %>% cohens_d(score ~ group, var.equal = FALSE)
...
# Bar-plot for 1 grouping variable
ggbarplot(df_sum_alpha_theta_beta_16_10_c1, x = "group", y = "score", 
          ylab='mean Alpha+Beta+Theta score',
          fill = group.colors,
          add = c("mean_se"),
          error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
...
Wlx.test.c1 <- df_sum_alpha_theta_beta_16_10_c1 %>% 
...
# Effect size
df_sum_alpha_theta_beta_16_10_c1 %>% wilcox_effsize(score ~ group)

# Bar-plot for 1 grouping variable
ggbarplot(df_sum_alpha_theta_beta_16_10_c1, x = "group", y = "score", 
          ylab='median Alpha+Beta+Theta score', fill=group.colors,
          add = c("median"),
          #error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
  #geom_jitter(size=1, width=0.1)+ # add subjects
...
df_sum_alpha_theta_beta_16_10_c2 <- data.frame(id, group, c2_During) # create df
names(df_sum_alpha_theta_beta_16_10_c2)[names(df_sum_alpha_theta_beta_16_10_c2) == 
                                    'c2_During'] <- 'score'
#df_sum_alpha_theta_beta_16_10_c2 <- df_sum_alpha_theta_beta_16_10_c2[-c(18), ] # remove outlier-18
df_sum_alpha_theta_beta_16_10_c2$score <- df_sum_alpha_theta_beta_16_10_c2$score*1e13 # normalization

# Summary stats
df_sum_alpha_theta_beta_16_10_c2 %>%
  group_by(group) %>%
  get_summary_stats(score, type = "mean_sd")
...
df_sum_alpha_theta_beta_16_10_c2 %>%
  ...
  shapiro_test(score)
# Draw a qq plot by group
ggqqplot(df_sum_alpha_theta_beta_16_10_c2, x = "score", facet.by = "group")
# Check the equality of variances
df_sum_alpha_theta_beta_16_10_c2 %>% levene_test(score ~ group) # p should be >0.05
...
# Compute Weltch T-test, which is the safer one
ttest.c2 <- df_sum_alpha_theta_beta_16_10_c2 %>% 
  t_test(score ~ group, paired = FALSE) %>%
  add_significance()
ttest.c2

# Effect size
df_sum_alpha_theta_beta_16_10_c2 %>% cohens_d(score ~ group, var.equal = FALSE)

# Report
# Auto-compute p-value label positions
ttest.c2 = ttest.c2 %>% add_xy_position(x = "group")
# Bar-plot for 1 grouping variable
ggbarplot(df_sum_alpha_theta_beta_16_10_c2, x = "group", y = "score", 
          ylab='mean Alpha+Beta+Theta score',
          fill = group.colors,
          add = c("mean_se"),
          error.plot = 'upper_errorbar',
...
Wlx.test.c2 <- df_sum_alpha_theta_beta_16_10_c2 %>% 
  wilcox_test(score ~ group, paired = FALSE,
              p.adjust.method = "BH") %>%
  add_significance()
Wlx.test.c2
...
# Effect size
df_sum_alpha_theta_beta_16_10_c2 %>% wilcox_effsize(score ~ group)
...
df_sum_alpha_theta_beta_16_10_c3 <- data.frame(id, group, c3_After) # create df
names(df_sum_alpha_theta_beta_16_10_c3)[names(df_sum_alpha_theta_beta_16_10_c3) == 
                                    'c3_After'] <- 'score'
#df_sum_alpha_theta_beta_16_10_c3 <- df_sum_alpha_theta_beta_16_10_c3[-c(18), ] # remove outlier-18
df_sum_alpha_theta_beta_16_10_c3$score <- df_sum_alpha_theta_beta_16_10_c3$score*1e13 # normalization

# Summary stats
df_sum_alpha_theta_beta_16_10_c3 %>%
  group_by(group) %>%
  get_summary_stats(score, type = "mean_sd")
...
df_sum_alpha_theta_beta_16_10_c3 %>%
  ...
  shapiro_test(score)
# Draw a qq plot by group
ggqqplot(df_sum_alpha_theta_beta_16_10_c3, x = "score", facet.by = "group")
# Check the equality of variances
df_sum_alpha_theta_beta_16_10_c3 %>% levene_test(score ~ group) # p should be >0.05
...
# Compute Weltch T-test, which is the safer one
ttest.c3 <- df_sum_alpha_theta_beta_16_10_c3 %>% 
  t_test(score ~ group, paired = FALSE) %>%
  add_significance()
ttest.c3

# Effect size
df_sum_alpha_theta_beta_16_10_c3 %>% cohens_d(score ~ group, var.equal = FALSE)
...
# Bar-plot for 1 grouping variable
ggbarplot(df_sum_alpha_theta_beta_16_10_c3, x = "group", y = "score", 
          ylab='mean Alpha+Beta+Theta score',
          fill = group.colors,
          add = c("mean_se"),
          error.plot = 'upper_errorbar',
...
Wlx.test.c3 <- df_sum_alpha_theta_beta_16_10_c3 %>% 
  wilcox_test(score ~ group, paired = FALSE,
              p.adjust.method = "BH") %>%
  add_significance()
Wlx.test.c3
...
# Effect size
df_sum_alpha_theta_beta_16_10_c3 %>% wilcox_effsize(score ~ group)
...
ggbarplot(df_sum_alpha_theta_beta_16_10_c3, x = "group", y = "score", 
  ...
  stat_pvalue_manual(Wlx.test.c3, label = "p = {p}", 
                     y.position = 650, # set 4000 if geom_jitter
                     tip.length = 0.03, size=3,
                     step.increase = 0.2)+
  labs( # add labels
    title = "?????????????????? Alpha+Beta+Theta-????????????????????",
