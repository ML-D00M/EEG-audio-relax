...
# 1 condition - Before
alpha_15Hsub_1=readMat('alpha_15Hsub_cond_1.mat')$alpha.sub
alpha_15Hsub_1=as.vector(alpha_15Hsub_1)
# 2.1 condition - Hypno: Intro
alpha_15Hsub_2_1=readMat('alpha_15Hsub_cond_2_1.mat')$alpha.sub
alpha_15Hsub_2_1=as.vector(alpha_15Hsub_2_1)
# 2.2 condition - Hypno: Body
alpha_15Hsub_2_2=readMat('alpha_15Hsub_cond_2_2.mat')$alpha.sub
alpha_15Hsub_2_2=as.vector(alpha_15Hsub_2_2)
# 2.3 condition - Hypno: Mind
alpha_15Hsub_2_3=readMat('alpha_15Hsub_cond_2_3.mat')$alpha.sub
alpha_15Hsub_2_3=as.vector(alpha_15Hsub_2_3)
# 2.4 condition - Hypno: Negative
alpha_15Hsub_2_4=readMat('alpha_15Hsub_cond_2_4.mat')$alpha.sub
alpha_15Hsub_2_4=as.vector(alpha_15Hsub_2_4)
# 2.5 condition - Hypno: Positive
alpha_15Hsub_2_5=readMat('alpha_15Hsub_cond_2_5.mat')$alpha.sub
alpha_15Hsub_2_5=as.vector(alpha_15Hsub_2_5)
# 3. condition - After
alpha_15Hsub_3=readMat('alpha_15Hsub_cond_3.mat')$alpha.sub
alpha_15Hsub_3=as.vector(alpha_15Hsub_3)
...
id <- sprintf("% d", 1:15)
c1_Before <- c(alpha_15Hsub_1)
c2.1_Intro <- c(alpha_15Hsub_2_1)
c2.2_Body <- c(alpha_15Hsub_2_2)
c2.3_Mind <- c(alpha_15Hsub_2_3)
c2.4_Negative <- c(alpha_15Hsub_2_4)
c2.5_Positive <- c(alpha_15Hsub_2_5)
c3_After <- c(alpha_15Hsub_3)

df_alpha_15H <- data.frame(id, c1_Before, c2.1_Intro, c2.2_Body, c2.3_Mind, c2.4_Negative, c2.5_Positive, c3_After)

# Gather the columns c1_Before - c3_After into long format.
# Convert id and cond into factor variables
df_alpha_15H <- df_alpha_15H %>%
...
df_alpha_15H$score <- df_alpha_15H$score*1e13
...
ggqqplot(df_alpha_15H, "score", facet.by = "cond")
...
# Effect size
df_alpha_15H %>% wilcox_effsize(score ~ cond)

# Report

# Bar-plot with p-values for 1 grouping variable - Cond
Wlx.test = Wlx.test %>% add_xy_position(x = "cond")
ggbarplot(df_alpha_15H, x = "cond", y = "score", 
          ylab='median Alpha score', fill=cond.colors,
          add = c("median"),
  ...
  stat_pvalue_manual(Wlx.test, label = "p = {p}", 
                     ...
                     hide.ns = TRUE,
                     step.increase = 0.2)+
  coord_cartesian(ylim = c(0, 20))+ # to set a limit to y value
  labs( # add labels
    title = "?????????????????? Alpha-????????????????????",
    subtitle = "?????? ???????????? ???????????? - ????, ?????????? ?? ???? ?????????? 5 ???????????????????? ??????????-????????????",
...
# Effect size
df_alpha_15H %>% cohens_d(score ~ cond, paired = TRUE, var.equal = FALSE)
...
ttest = ttest %>% add_xy_position(x = "cond")
ggbarplot(df_alpha_15H, x = "cond", y = "score", 
          ylab='mean Alpha score', fill=cond.colors,
          add = c("mean_se"),
          error.plot = 'upper_errorbar',
          position = position_dodge(width=0.8))+
  #geom_jitter(size=1, width=0.1)+ # add subjects
